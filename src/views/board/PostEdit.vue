  ...
  <v-container>
    <v-card elevation="3" width="80%" class="mx-auto my-16">
      ...
      <v-card-text>
        <v-row>
          <v-col class="my-0">
            <v-text-field label="제목" variant="underlined" v-model="post.postTtl"></v-text-field>
          </v-col>
        </v-row>
        <v-row class="my-0">
          <v-col>
            <v-text-field label="작성자" readonly v-model="post.userNm" variant="underlined"></v-text-field>
          </v-col>
          <v-col>
            <v-text-field label="작성일시" readonly v-model="post.rgstDt" variant="underlined"></v-text-field>
          </v-col>
        </v-row>
        <v-row class="my-0">
          <v-col>
        ...
        <v-row class="my-0">
          <v-layout v-if="fileInfo.length != 0" row wrap>
            <v-flex v-for="file in fileInfo" :key="file.fileId" xs4 class="mr-3">
              ...
              <img :src="`/api/v1/files/${file.fileId}`" class="image" alt="lorem" width="100%" height="100%" />
            </v-flex>
          </v-layout>
          <v-layout v-else>
        ...
        <v-row class="my-0">
          <v-col>
            <v-textarea label="내용" v-model="post.postCnt" variant="underlined"></v-textarea>
          </v-col>
        </v-row>
      </v-card-text>
      <v-card-actions>
